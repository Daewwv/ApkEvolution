...
const KEY="progresslife_v031_state";
const BUILD_VERSION="v0.3.1";
...
{id:"m3",type:"Aplicação",title:"Analisar seus últimos gastos",why:"Veja onde gastou, o que poderia ter evitado e qual ajuste pequeno melhora sua semana.",cost:2,minutes:10,done:false,persistent:true}],
...
function completeMission(id){
 ...
 if(m.done){
   ...
   if(m.id==="m1")state.savings=Math.max(0,state.savings-10);
   if(m.id==="m2")state.completedLessons=Math.max(0,state.completedLessons-1);
   state.evidence.unshift(`Desmarcou: ${m.title}`);
 }else{
   m.done=true;
   state.xp+=25;
   if(m.id==="m1")state.savings+=10;
   if(m.id==="m2")state.completedLessons+=1;
   state.evidence.unshift(`Concluiu: ${m.title}`);
   state.missedDays=0;
 }
 save();render();
}
function advanceBlock(){return `<section class="card success stack" style="margin-top:16px"><div><p class="tag">Modo Avanço</p><h3>Você concluiu o essencial de hoje.</h3><p class="muted">Se ainda tiver energia, escolha um avanço. Se não tiver, descansar também é uma decisão inteligente.</p></div><button class="primary" onclick="addImpact()">Fazer ação de impacto</button><button class="secondary" onclick="go('pending')">Resolver pendência</button><button class="secondary" onclick="showToast('Descanso registrado. Amanhã continuamos com constância.')">Descansar e manter constância</button></section>`}
